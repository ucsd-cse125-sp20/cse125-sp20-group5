...
Scene::Scene(ClientParams& config) : config(config)
{
	...
	textInWorld = new TextUI(textProgram->GetProgramID(), FontType::CHUNK);
...
void Scene::draw(const glm::mat4 &viewProjMat)
{
	...
	textInWorld->renderTextInWorld("Lv " + to_string(2),
		viewProjMat, glm::translate(glm::vec3(0,5,0)), 0.02f, glm::vec3(1.0, 0.0f, 0.0f));
